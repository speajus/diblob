...
import { z } from 'zod';
import type { TelemetryConfig, TelemetryLokiConfig } from './blobs.js';
...
export const TelemetryLokiConfigSchema = z
		.object({
			host: z
				.string()
				.url()
				.optional()
				.describe('Loki base URL; when omitted, Loki logging is disabled.'),
			labels: z
				.record(z.string(), z.string())
				.optional()
				.describe('Static labels applied to Loki log streams.'),
			level: z
				.string()
				.optional()
				.describe('Minimum log level for Loki transport.'),
			interval: z
				.number()
				.int()
				.positive()
				.optional()
				.describe('Batch push interval for Loki in milliseconds.'),
			json: z
				.boolean()
				.optional()
				.describe('Whether to send logs to Loki as JSON.'),
			batching: z
				.boolean()
				.optional()
				.describe('Whether to batch logs before sending to Loki.'),
			enabled: z
				.boolean()
				.optional()
				.describe('Explicit switch to enable Loki logging.'),
			 }) satisfies ConfigSchema<TelemetryLokiConfig>;
