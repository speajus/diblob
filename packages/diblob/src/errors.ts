...
export class BlobNotRegisteredError extends DiblobError {
  constructor(
    ...
    const containerContext = containerInfo ? ` in container "${containerInfo}"` : '';
    super(
      `Blob[${blobName}] not registered${containerContext}. Call container.register() first.\n\n` +
...
/**
 * Error thrown when a list blob is accessed or mutated before being registered.
 */
export class ListBlobNotRegisteredError extends DiblobError {
  constructor(
    public readonly blobName: string,
    public readonly operation: 'access' | 'mutation' = 'access',
  ) {
    const opDesc = operation === 'mutation' ? 'mutations' : 'use';
    super(
      `List blob "${blobName}" must be registered with a container before ${opDesc}.\n\n` +
        `Array operations require the blob to be registered.\n\n` +
        `To fix this:\n` +
        `  container.register(${blobName}, () => []);\n\n` +
        `Then you can ${operation === 'mutation' ? 'mutate' : 'access'}:\n` +
        `  ${blobName}.push('item');\n`,
    );
...
/**
 * Error thrown when a container does not support blob property resolution.
 * This typically indicates using a blob with an incompatible container implementation.
 */
export class ContainerNotSupportedError extends DiblobError {
  constructor(public readonly operation: 'property' | 'instance') {
    const opDesc = operation === 'property' ? 'property resolution' : 'instance resolution';
    super(
      `Container does not support blob ${opDesc}.\n\n` +
        `This error occurs when using a blob with an incompatible container implementation.\n\n` +
        `Make sure you are using a container created with createContainer().\n`,
    );
    this.name = 'ContainerNotSupportedError';
  }
}
