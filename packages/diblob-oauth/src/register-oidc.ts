		          ...
		          return {
			        ...
			        async buildEndSessionUrl(input) {
			          ...
			          if (!endSessionEndpoint) {
			            throw new Error('OIDC end_session_endpoint is not configured on issuer');
			          }
			          const url = new URL(endSessionEndpoint);
			          const redirectUri =
			            input.postLogoutRedirectUri ?? config.redirectUris[0];
			          if (redirectUri) {
			            // Use both the standard OIDC parameter and Cognito-specific logout_uri
