...
appLogger.info('OAuth Cognito example initialized', {
...
const server = createServer(async (req, res) => {
	...
	if (req.method === 'GET' && url.pathname === '/login') {
		const authorizationUrl = await client.fetchAuthorizationUrl({
			state: 'demo-state',
		});
		res.statusCode = 302;
	...
	if (req.method === 'GET' && url.pathname === '/callback') {
		...
		try {
			...
			// Create a simple server-side session keyed by a cookie.
